...
def get_models(selected=0):
    ...
    for line in lines:
        if line:  # Ignore empty lines
            ...
            if ":" in parts[0].strip():
                selected_file = i == selected
                splitmodel = parts[0].strip().split(":")
...
def index():
    ...
    if request.method == 'POST':
        ...
        #print(type(answer))
        return render_template('index2.html', answer=answer, calculated_time=taken_time, options=conv_history, models=models)
    else:
        return render_template('./index2.html', options=conv_history, models=models)
